...
The service layer now covers reconnect replay, leased timeouts, explicit sit-out/return/leave state, time-bank consumption, ROE scheduling, wallet-bound WebSocket commands, connection-encrypted hole-card envelopes, append-only snapshots, deterministic randomized tests, and a disposable 16-table concurrency/failure-injection baseline. Tournament rules, collusion/bot detection, production hand-history UX, attested-dealer wiring, sustained target-load soak/network chaos, and independent review remain incomplete. Those gaps keep real-value mode disabled even though the mathematical and orchestration cores are implemented.
