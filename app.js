...
function walletXStocksGrid(walletHoldings, selectedSymbol) {
  ...
  return `<div class="xstock-portfolio-grid">${holdings.map((holding) => {
    ...
    return `<button class="xstock-position ${holding.detected ? "is-held" : ""} ${holding.symbol === selectedSymbol ? "is-selected" : ""}" data-action="select-swap-output" data-symbol="${escapeHtml(holding.symbol)}" aria-pressed="${holding.symbol === selectedSymbol}">${assetLogo(asset)}<span><strong>${escapeHtml(holding.symbol)}</strong><small>${escapeHtml(holding.name)}</small></span><span class="position-quantity"><strong>${holding.detected ? escapeHtml(holding.displayAmount ?? "Held") : "Get"}</strong><small>${holding.detected ? "in wallet" : "via Jupiter"}</small></span></button>`;
  }).join("")}</div>`;
}
...
function investmentsView() {
  ...
  const error = state.investments.error ? `<div class="inline-error"><strong>Wallet market unavailable</strong><span>${escapeHtml(state.investments.error)}</span></div>` : "";
  return pageShell(`<section class="investment-hero wallet-market-hero"><div><span class="eyebrow">Jupiter-powered · Solana self-custody</span><h1>Hold the market.<br><em>Keep the keys.</em></h1><p>See the Core 10 xStocks already in your connected wallet and swap SOL or USDC into another. Every transaction is reviewed and signed in your wallet.</p></div><div class="ownership-seal wallet-seal"><span>SOLANA</span><i></i><span>WALLET</span><strong>YOU</strong></div></section>${error}<section class="wallet-market-board ${loading ? "is-loading" : ""}"><article class="wallet-vault"><header><div><span class="rail-index">YOUR WALLET / CORE 10</span><h2>Choose an xStock</h2></div>${investmentRailStatus(Boolean(state.profile && !state.profile.isGuest), `${heldCount} held`, "Wallet required")}</header><p>Select any tile to prepare a Jupiter swap. Balances are read directly from Solana mainnet without requesting wallet permissions.</p>${walletXStocksGrid(portfolio?.walletHoldings, selected.symbol)}<footer><span>${escapeHtml(shortWallet(state.profile?.wallet))} · public balance scan</span><button class="text-action" data-action="refresh-investments">Refresh balances ↻</button></footer></article><aside class="swap-studio wallet-swap-studio"><div class="swap-intro"><span class="eyebrow">Direct wallet swap</span><h2>Get ${escapeHtml(selected.symbol)}.</h2><p>Jupiter builds the route. Your connected wallet verifies and signs the final Solana transaction; xPoker never receives your private key.</p>${investmentRailStatus(Boolean(swaps?.enabled), "Jupiter ready", "Direct swap setup pending")}</div><div class="swap-composer"><div class="swap-leg"><label>You pay</label><div><input id="swap-amount" inputmode="decimal" value="${escapeHtml(state.swapAmount)}" aria-label="Swap amount"/><select id="swap-input" aria-label="Input asset">${(swaps?.supportedInputs || [{ symbol: "SOL" }, { symbol: "USDC" }]).map((asset) => `<option value="${asset.symbol}" ${state.swapInput === asset.symbol ? "selected" : ""}>${asset.symbol}</option>`).join("")}</select></div></div><span class="swap-arrow">↓</span><div class="swap-leg receive"><label>You receive</label><div class="swap-output is-static">${assetLogo(selected)}<span><strong>${escapeHtml(selected.symbol)}</strong><small>${escapeHtml(selected.name)}</small></span><b>Selected</b></div></div><button class="btn btn-accent swap-quote-button" data-action="quote-swap" ${swaps?.enabled ? "" : "disabled"}>Get Jupiter route</button>${swaps?.enabled ? "" : `<a class="jupiter-fallback" href="https://jup.ag/swap/SOL-${encodeURIComponent(selected.mint || "")}" target="_blank" rel="noopener noreferrer">Open this pair on Jupiter ↗</a>`}${swapReview}</div></aside></section><div class="investment-boundary"><strong>Wallet-owned, game-separated.</strong><span>xStocks acquired or displayed here stay in the connected wallet. Poker tables continue to use free, non-transferable demo credits.</span></div>`);
}
...
function handleAction(event) {
  ...
  if (action === "select-swap-output") { state.selectedAsset = state.assets.find((asset) => asset.symbol === target.dataset.symbol) || state.selectedAsset; state.swapOrder = null; render(); }
